...
const insertStrayReport = (reportData, callback) => {
  ...
  db.run(sql, [
    reportData.reportId, reportData.reporterCui, reportData.reporterName,
    reportData.reporterPhone, reportData.reporterEmail, reportData.latitude,
    reportData.longitude, reportData.address, reportData.zone, reportData.breed,
    reportData.size, reportData.colors, reportData.temperament, reportData.condition,
    reportData.gender, reportData.estimatedAge, reportData.healthStatus,
    reportData.urgency, reportData.status, reportData.description,
    reportData.hasCollar, reportData.isInjured, reportData.needsRescue,
...
const updateStrayReportStatus = (reportId, newStatus, updatedBy, reason, callback) => {
...
router.get('/', (req, res) => {
  const { urgency, condition, size, temperament, recent, lat, lng, radius } = req.query
...
router.post('/', upload.single('photo'), async (req, res) => {
  try {
    ...
    const reportId = `SR-${new Date().getFullYear()}-${uuidv4().substring(0, 8).toUpperCase()}`
    ...
    const cuiCheck = await new Promise((resolve, reject) => {
      db.get('SELECT cui FROM pets WHERE cui = ?', [reporterCui], (err, row) => {
        if (err) reject(err)
        else resolve(row)
      })
    })

-- =====================================================
-- INSTRUCCIONES: COPIAR Y PEGAR EN phpMyAdmin
-- =====================================================
-- 
-- PASOS:
...
-- 4. Ctrl+A (seleccionar todo este archivo)
-- 5. Ctrl+C (copiar)
-- 6. Ctrl+V (pegar en phpMyAdmin)
-- 7. Clic en "Continuar"
--
-- =====================================================
-- SCRIPT COMPLETO DE BASE DE DATOS
-- Sistema de Registro de Mascotas - Municipalidad Provincial de Puno
-- =====================================================
-- 
-- CONTENIDO:
-- ✓ 15 tablas completas
-- ✓ 36+ razas (incluye 7 razas peligrosas)
-- ✓ Sistema de pagos para razas peligrosas (S/.52.20)
-- ✓ Datos iniciales, vistas y triggers
--
-- RAZAS PELIGROSAS (requieren pago S/.52.20):
-- • Pit Bull Terrier
-- • Dogo Argentino  
-- • Fila Brasilero
-- • Tosa Japonesa
-- • Bul Mastiff
-- • Doberman
-- • Rottweiler (Rotweller)
--
-- Información de pago:
-- Cuenta: 00701-038452
-- Teléfono: (051) 601000, Anexo: 2071
-- Correo: ogambiental@munipuno.gob.pe
--
-- =====================================================
...
CREATE TABLE IF NOT EXISTS breeds (
    ...
    INDEX idx_name (name)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de razas de perros';
...
CREATE TABLE IF NOT EXISTS colors (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_name (name)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de colores con códigos hex';
...
CREATE TABLE IF NOT EXISTS sizes (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_code (code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de tamaños (pequeño, mediano, grande)';
...
CREATE TABLE IF NOT EXISTS temperaments (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_code (code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de temperamentos con colores asociados';
...
CREATE TABLE IF NOT EXISTS report_conditions (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_code (code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de condiciones (callejero, perdido, abandonado)';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Catálogo de niveles de urgencia con prioridad';

-- =====================================================
-- TABLAS PRINCIPALES DE USUARIOS Y PROPIETARIOS
-- =====================================================

-- Tabla de propietarios/adoptantes
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Tabla de propietarios/adoptantes de mascotas';
...
CREATE TABLE IF NOT EXISTS pets (
    id INT AUTO_INCREMENT PRIMARY KEY,
    cui VARCHAR(20) UNIQUE NOT NULL COMMENT 'Código Único de Identificación',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Tabla principal de mascotas registradas';

-- =====================================================
-- TABLAS ESPECIALIZADAS DE MASCOTAS
-- =====================================================
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Registros de salud de mascotas';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Documentos y archivos de mascotas';

-- Tabla de pagos para razas potencialmente peligrosas
-- IMPORTANTE: Las razas potencialmente peligrosas requieren un pago de S/.52.20
-- a la Municipalidad Provincial de Puno
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Pagos para razas potencialmente peligrosas';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Historial de vacunaciones';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Relación mascotas-colores';

-- =====================================================
-- TABLAS DE REPORTES DE PERROS CALLEJEROS
-- =====================================================

-- Tabla de reportes de perros callejeros
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Reportes de perros callejeros';

-- Tabla pivote para colores de reportes de callejeros
CREATE TABLE IF NOT EXISTS stray_report_colors (
    ...
    INDEX idx_color (color_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Relación reportes-colores';

-- =====================================================
-- DATOS INICIALES DE CATÁLOGOS
-- =====================================================

-- Insertar razas comunes y RAZAS POTENCIALMENTE PELIGROSAS
...
-- RAZAS POTENCIALMENTE PELIGROSAS (Requieren pago de S/.52.20)
('Pit Bull Terrier', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Pitbull', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('American Pit Bull Terrier', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Dogo Argentino', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Fila Brasilero', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Fila Brasileiro', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Tosa Japonesa', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Tosa Inu', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Bul Mastiff', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Bull Mastiff', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Bullmastiff', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Doberman', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Dobermann', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Doberman Pinscher', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Rottweiler', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20'),
('Rotweller', 'Raza potencialmente peligrosa - Requiere pago de S/.52.20');
...
-- Vista completa de mascotas con toda la información relacionada
...
-- Trigger para generar CUI automáticamente
DELIMITER $$
...
-- =====================================================
-- INFORMACIÓN IMPORTANTE PARA RAZAS PELIGROSAS
-- =====================================================
-- 
-- RAZAS POTENCIALMENTE PELIGROSAS que requieren pago:
-- 1. Pit Bull Terrier
-- 2. Dogo Argentino
-- 3. Fila Brasilero
-- 4. Tosa Japonesa
-- 5. Bul Mastiff (Bull Mastiff)
-- 6. Doberman
-- 7. Rottweiler (Rotweller)
--
-- INFORMACIÓN DE PAGO:
-- Monto: S/.52.20
-- Cuenta: 00701-038452
...
--
-- DATOS REQUERIDOS EN EL RECIBO:
-- - N° RECIBO DE CAJA
-- - EMISIÓN (Fecha)
-- - NOMBRE / RAZÓN SOCIAL
-- - MONTO PAGADO (S/.52.20)
--
-- =====================================================
